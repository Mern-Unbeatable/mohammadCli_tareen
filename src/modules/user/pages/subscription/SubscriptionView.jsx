...
const SubscriptionView = () => {
  ...
  const { plans, plansLoading, saving, error } = useSelector(
  ...
  const trialDaysLeft = profileUser?.trialDaysLeft ?? 0;
  const isTrial = profileUser?.membershipStatus === 'trial';

  const handleSubscribe = async () => {
    const apiPlan = PLAN_API[cycle] || String(cycle).toUpperCase();
    const result = await dispatch(subscribe(apiPlan));
    if (subscribe.fulfilled.match(result)) {
